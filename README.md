...
Running alongside the core components from Hyperledger Sawtooth, BBR Chain
includes a number of elements customizing the blockchain and user interaction
with it:

- a **transaction processor** which handles BBR Chain transaction logic
- a **server** which provides an HTTP/JSON API for BBR Chain actions
...
required to run BBR Chain components.
...
- The BBR Chain REST API will be at **http://localhost:8020**
...
use all BBR Chain components. However if you want to run any additional
...
immediately be reflected in BBR Chain components without having to rebuild
...
The latest documentation for Sawtooth BBR Chain is available within this
...
Sawtooth BBR Chain documentation in the [docs](docs)
